...
@description Experimental function to "clean" list input where
duplicates exist (that would otherwise prevent `pbindlist`). Useful for
queries that may have overlapping instances of the same data, for
instance a list of SoilProfileCollections where each list element
contains profiles gathered from a set of (potentially overlapping)
extents.
