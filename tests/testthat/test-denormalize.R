...
# and number of horizons to number of values in denormalize result
test_that("denormalize result is 1:1 with horizons", {
  expect_equal(length(sp1), sum(table(sp1$sitevar)))  # check that the input is 1:1 with site
  expect_equal(nrow(sp1), sum(table(sp1$hz.sitevar))) # check that the output is 1:1 with horizon
})