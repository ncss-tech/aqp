...
Version 2.0 of the {aqp} package provides two algorithms for avoiding overlapping annotation, available in `plotSPC()` for the adjustment of horizon depth annotation, and for general use via `fixOverlap()`.
...
An electrostatic simulation applies forces of repulsion between labels that are within a given threshold (e.g. overlapping) and forces of attraction to a uniformly spaced sequence to iteratively perturb affected labels until either no overlap is reported, or a maximum number of iterations (`maxIter`) has been reached. Label adjustment solutions are deterministic and can be controlled by additional arguments to `electroStatics_1D()`. The most common adjustment is charge density, `q`. See comparisons below.

```{r fig.width=8, fig.height=5, echo = FALSE}
par(mar = c(0, 0, 0, 0))
plotSPC(s, fixOverlapArgs = list(method = 'E'))
title('Label Adjustment by Electrostatic Simulation', line = -1.5)
```

### Simulated Annealing
This approach makes small adjustments to label position until overlap is removed, or until `maxIter` is reached. Rank order and boundary conditions are preserved. The underlying algorithm is based on simulated annealing. Label placement solutions are non-deterministic, use `set.seed()` if repeatability is important. The "cooling schedule" parameters `T0` and `k` can be used to tune the algorithm for specific applications. These additional arguments are passed to `SANN_1D()`.
...
In general, as the complexity increases (larger thresholds, more overlap), `q` needs to be larger.
```{r fig.width=8.5, fig.height=5}
