...
The `quickSPC()` function can be used to rapidly prototype `SoilProfileCollection` objects based on a simple `list` template, or one of several text-based templates:
  ...
  Format 2 is interpreted as a horizon sequence delimited by '|'. Horizon thickness is proportional to replication of horizon designation and scaled by the `interval` argument to `quickSPC()`. Profile ID is created via `digest::digest(..., algo = 'xxhash32')`. Iteration over templates in this format is automatic when `x` is a character vector of `length > 1`.
...
plotSPC(s, name.style = 'center-center', 
        cex.names = 0.66, plot.depth.axis = FALSE,
        hz.depths = TRUE)
