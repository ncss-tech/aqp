...
Following LEFT JOIN of new data (`site<-`, `horizons<-`) when site and horizon IDs are integers, and some new attributes contain `NA`, the object's site/horizon order _must_ be preserved. 
 ...
 - No order corrections are needed for `merge.data.table` (that applies when the slots are `data.table`s). Other than ensuring that the `data.frame` `j` index was used unambiguously, `data.table` was a drop-in replacement.
