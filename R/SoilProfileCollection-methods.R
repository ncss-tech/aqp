...
#' @description `grepSPC()` is a shorthand function for subsetting `SoilProfileCollection` objects. For example, by `filter(grepl(spc, ...))` or `filter(stringr::str_detect(spc, ...))`. It provides pattern matching for a single text-based site or horizon level attribute.
#' @param object A SoilProfileCollection
