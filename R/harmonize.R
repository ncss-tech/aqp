...
#' 
#' # make test data 
#' spc1 <- aqp::union(lapply(LETTERS[1:5], random_profile, SPC = TRUE))
#' spc2 <- aqp::union(lapply(letters[1:5], random_profile, SPC = TRUE))
#' 
#' h4 <- aqp::union(list(harmonize(spc1, list(foo = c(transect1 = "p4"))),   # foo is p4 in dataset 1
...
setMethod("harmonize", signature(x = "SoilProfileCollection"), 
  ...
  # nested loop -- iterating over all profiles as many unique levels we have in elements of x.names
   aqp::union(profileApply(x, function(p) {
      aqp::union(lapply(1:length(x.sets), function(i) {
