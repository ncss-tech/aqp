...
# initializer function for SoilProfileCollection class
...
definition=function(object, value)
  ...
  if (inherits(value, "formula")) 
	...
	if(length(user_id) == 1)
	  ...
	  # I tried to find the columns which have one value per id and set them at site_data
	  # but later on I realized that horizon data could have the same value for each depth
	  # so I think site_data cols should be explicited by the user.
...
definition=function(object)
  {
  return(ldply(object@collection, function(x){x@site_data}))
  }
)

##
## horizon_data() accessor
##
setGeneric('horizon_data', package='aqp', def=function(object, value) 
  {
  standardGeneric('horizon_data')
  }
)

# simple case, just a single SoilProfile
setMethod(f='horizon_data', signature='SoilProfile',
definition=function(object)
  {
  return(object@horizon_data)
  }
)

setMethod(f='horizon_data', signature='SoilProfileCollection',
definition=function(object)
  {
  return(ldply(object@collection, function(x){x@horizon_data}))
  }
)
...
definition=function(object, value)
	...
	# otherwise we just take the first row
	else
	  {
	    ...
	    if (length(id_error) == 0) {
	      ...
	      stop('Those values are not site-specific - PLACE BETTER ERROR MESSAGE HERE!')
	  }
  ...
  # in this case the user is supplying a 1-row dataframe with information
	...
	if (nrow(value) == 1)
	  {
	  # I implemented it as a possibility to add a property. This is to be discussed!
	  object@site_data <- cbind(object@site_data, value)
	  }
	else 
	  stop('The dimensions of the data.frame does not match as a site_data. Did you  mean horizon_data?')
	}
  else
	stop('invalid initialization for SoilProfile object')
...
definition=function(object, value)
  ...
  # in this case the user is supplying a 1-row dataframe with information
	...
	if (nrow(value) == length(object))
	  {
	  # I implemented it as a possibility to add a property. This is to be discussed!
#	  foo@collection[[i]]@site_data
	  affect <- function(object, data){
	    object@site_data <- cbind(object@site_data, data)
	    object
	  }
	  llply(foo@collection, affect, data=a)
	  object@site_data <- cbind(object@site_data, value)
	  }
	else 
	  stop('The dimensions of the data.frame does not match with the number of profiles in the SoilProfileCollection.')
	}
  else
	stop('invalid initialization for SoilProfile object')
