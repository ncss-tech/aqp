...
# 
# quantile(sp1$bound_sd, na.rm=T)
# p <- sp1[3]

# opinion: this method is most "believable" when used to _gently_ permute the data, on the order of moving boundaries a few cm in either direction. The nice thing about it is it can leverage semiquantitative (ordered factor) levels of boundary distinctness/topography for the upper and lower boundary of individual horizons, given a set of assumptions to convert classes to a "standard deviation".

# If you imagine a normal curve with its mean centered on the vertical (depth axis) at a RV horizon depth of 18cm. By the Empirical Rule for Normal distribution, two "standard deviations" above or below that RV depth represent 95% of the "volume" of the boundary. So, a standard deviation of 1-2cm would yield a "boundary thickness" in the 3-5cm range ("clear" distinctness class). Of course, boundaries are not symmetrical and this is at best an approximation for properties like organic matter nutrients or salts that can have strong depth-dependence within horizons. Also, boundary topography is non-uniform. There are definitely ways to implement other distributions, but invokes more detailed assumptions about field data that are generally only semi-quantiative or are not available. 

# I plan to play around with implementing boundary topography as a second hierarchical level (e.g. trig-based random functions), but think that distinctness captures the "uncertainty" about horizon separation at a specific "point" on the ground (or line in the profile) sufficiently for my taxonomic simulations for individual pedons
